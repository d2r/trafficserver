                ...
                va_list ap) const
{
  ...
  LocalBufferWriter<BUF_CAPACITY> format_writer;
  ...
  //////////////////////
  {
    ...
    format_writer.write('[');
    format_writer.write(buffer + 4, std::min(strlen(buffer + 4), sizeof(timestamp_buf) - 4));
    format_writer.write("] ", 2);
  }

  size_t timestamp_end_offset = format_writer.extent();

  ///////////////////////
  // add the thread id //
  ///////////////////////
  {
    int num_bytes_written = snprintf(format_writer.auxBuffer(), format_writer.remaining(), "{0x%" PRIx64 "} ", (uint64_t)ink_thread_self());
  ...
  if (format_writer.data()[format_writer.extent() - 1] != '\n') {
    format_writer.write('\n');
  }
